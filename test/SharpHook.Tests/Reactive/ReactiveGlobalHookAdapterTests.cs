...
public sealed class ReactiveGlobalHookAdapterTests
{
    ...
    [Fact(DisplayName = "SimpleReactiveGlobalHook should not throw if the scheduler is null")]
    public void SchedulerNull()
    {
        var exception = Record.Exception(() => new ReactiveGlobalHookAdapter(new SimpleGlobalHook(), null));
        Assert.Null(exception);
    }

    private void RunHookAndWaitForStart(
        ReactiveGlobalHookAdapter hook,
